...
def create_base_plot(persistence_points, file_name, max_dim):
    # Create Plot
    fig, ax = plt.subplots()
    ...
    for point in persistence_points:
        ax.scatter(point[1][0], point[1][1], color=cmap[point[0]], alpha=0.75, s=70, zorder=10, clip_on=False)
    ax.set_title('Persistence Diagram of\n\'"{}\"'.format(file_name))
    handle_list = []
    for dim in list(range(0, max_dim + 1)):
        handle_str = mpatches.Patch(
            color=cmap[dim],
...
def plot_persdia_main(persistence_points, file_name, max_dim, extension='.png', save_dir=None,
                      save_plot=True, file_time=None, list_points=False, save_points=False, multi_run=False,
    ...
    if override_orig is None:
        ...
        if multi_run:
            save_plot_state(persistence_points, y_max_orig, y_max_tick, y_inf_tick, inf_bool, save_dir, file_time,
                            file_name, save_plot, save_points, list_points)
    elif not multi_run:
        ...
        # Create initial plot
        fig, ax = create_base_plot(persistence_points, file_name, max_dim)
...
def save_persistence_points(persistence_points, save_file_name):
    print('Saving Persistence Diagram Points to {}.txt'.format(save_file_name))
...
def save_plot_state(persistence_points, y_max_orig, y_max_tick, y_inf_tick, inf_bool, save_dir, file_time,
                    save_file_name, save_plot, save_points, list_points):
