...
def meshpy_from_file(file_name, file_path, meshpy_switch,
                     random_noise=False, verbose=True):
    """
    ...
    :param random_noise: Boolean to introduce random noise into decimal values of points
    ...
    if random_noise:
        ...
        for n, i in enumerate(meshinfo.points):
            print("{}: {}".format(n, i))
...
def plot_persdia_gudhi(gudhi_simplex_tree, file_name, meshpy_switch, show_legend=True,
                       extension='.png',
                       save_plot=True, file_time=None, save_dir=None, h1_h2_plot=False,
                       list_points=False, save_points=False, show_plot=True):
    """
    ...
    :param file_time: String time of when the main file was run
    :param save_dir: String of directory to save html file to
    :param h1_h2_plot: Only show H_1 and H_2 points on persistence diagram
    ...
    if save_points:
        with open(save_file_name + '.txt', 'w') as file:
            for line in gudhi_simplex_tree.persistence():
                file.write(f"{line}\n")
    if (file_time and save_dir is not None) and save_plot:
        plt.savefig('{}{}'.format(save_file_name, extension))
        print('\nPersistence Diagram saved to {}{}'.format(save_dir, extension))
    if show_plot:
        plt.show()
